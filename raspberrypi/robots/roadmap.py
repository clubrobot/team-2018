...
#short class just to be hashable (to stock history in a dict)
class cutLineHashable():
...
class RoadMap:
	...
	def cut_edges(self, cutline):
		cutline=cutLineHashable(cutline)
		if not (cutline in self.edgesCutted):
			self.edgesCutted[cutline]=dict()
		for edge in self.graph.es:
			source = self.graph.vs[edge.source]['coords']
			target = self.graph.vs[edge.target]['coords']
			if intersect((source, target), cutline.get()):
				self.edgesCutted[cutline][edge]=edge['weight']
				edge['weight'] = math.inf

	def restore_edges(self,cutline):
		cutline=cutLineHashable(cutline)
		if not (cutline in self.edgesCutted):
			return
		for edge, weight in self.edgesCutted[cutline]:
			edge['weight'] = weight
		del self.edgesCutted[cutline]
